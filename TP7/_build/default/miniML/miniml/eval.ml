...
let rec string_from_expr  = function
  ...
  | Try (x , y, z) -> raise (NotImplemented __LINE__)
  | Raise (x) -> raise (NotImplemented __LINE__)
...
let rec subst x byx e = match e with
  ...
  | Raise (_) -> raise (NotImplemented __LINE__)
  | Try (_,_,_) -> raise (NotImplemented __LINE__)
